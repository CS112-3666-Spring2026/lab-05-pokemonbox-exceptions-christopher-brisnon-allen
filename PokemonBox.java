public class PokemonBox {
	...
	public PokemonBox(Pokemon[] caught) throws IllegalArgumentException {
		if(caught == null || caught.length == 0) {
			throw new IllegalArgumentException("Invalid Pokemon array provided to PokemonBox. Exiting program.");
		}
	...
	public void add(Pokemon newPoke) throws PokemonAlreadyExistsException {
		// if pokemone exists, throw error
		if (this.hasPokemon(newPoke.getName())) {
			throw new PokemonAlreadyExistsException("Error: Pokemone already exists:", newPoke);
		} else {
			...
			try {
				...
				System.out.println("Error: trying to copy a null Pokemon in PokemonBox");
				System.exit(0);
			}
		}
	}
	...
	private Pokemon[] deepCopyArray(Pokemon[] p, int newLength) {
		...
		for (int i = 0; i < p.length; i++) {
			try {
				...
				System.out.println("ERROR: trying to copy null whie deep copying array.");
				System.exit(0);
			}
			}
